...
const AvailabilityOverlay = ({ active, closeOverlay, overlayInfo }) => {
	...
	const handleOccupancyString = () => {
		if (activeFloorplan === 0) setOccupancyString(`${headCountOne ? headCountOne : ""}`)
		if (activeFloorplan === 1) setOccupancyString(`${headCountTwo ? headCountTwo : ""}`)
		if (activeFloorplan === 2) setOccupancyString(`${headCountThree ? headCountThree : ""}`)
		if (activeFloorplan === 3) setOccupancyString(`${headCountFour ? headCountFour : ""}`)
	}
	...
	return (
		<Overlay active={active} aria-hidden={active}>
			<CloseOverlay aria-label="close floorplan overlay" onClick={closeOverlay}>
				<img aria-hidden src="/close.svg" alt="close availability overlay" />
			</CloseOverlay>
